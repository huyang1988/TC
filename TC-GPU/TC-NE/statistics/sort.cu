...
void graph::partition(){
	//step 1, evenly cut the upper CSR by using binary search in beg_pos
...
//	partDegree = new index_t*[PART_NUM];
	...
	offset[0] = 0;
	offset[PART_NUM] = edge_count;
	for(int i=1; i<PART_NUM; i++){
		index_t k=i*edge_count/PART_NUM;
//		cout<<"k="<<k<<"\n";
		vertex_t index = BinarySearch(k, beg_pos, 0, vert_count-1);
		offset[i] = beg_pos[index];
//		cout<<"part "<<i<<" cut at "<<index<<"\n";
	}
	vertex_t *destAdj = new vertex_t[edge_count];
	vertex_t *destHead= new vertex_t[edge_count];
//printGraph(vert_count, upperAdj, upperHead, beg_pos);
//	memcpy(destAdj,upperHead,edge_count*sizeof(int));
//	memcpy(destHead,upperAdj,edge_count*sizeof(int));
	...
	//reverse here	
		vertex_t* tempHead = &adj_list[offset[i]];
		vertex_t* tempAdj  = &head_list[offset[i]];
