...
//	vertex_t*	dev_head;
	...
	Edge*	workload  = mygraph->OrientedEdge;
	...
	for(int j=0; j<PART_NUM; j++){	
		...
		cout<<"totalEdgeCount = "<<totalEdgeCount<<endl;
...
//		H_ERR(cudaMemcpy(src_head,   head, totalEdgeCount*sizeof(vertex_t), cudaMemcpyHostToDevice) );
		...
		Edge*	dev_workload;		
		H_ERR(cudaMalloc(&dev_workload, totalEdgeCount*sizeof(Edge)) );
		H_ERR(cudaMemcpy(dev_workload,   workload, totalEdgeCount*sizeof(Edge), cudaMemcpyHostToDevice) );
...
void graph::DeviceCompute(int GPU_id, index_t Chunk_id){
	...
	Edge*		buffer		=gdata[GPU_id].EdgeBuffer[Chunk_id%2];
	...
	(	buffer,
		...
		0,
//		GPU_id*256*256/32,
		currentBufferSize,//totalEdgeCount,
//		upperEdgeCount,
		dev_count
	);
