...
void part_scan(int PART_id){


	index_t thd_count=0;	
	int i = PART_id;
	int GPU_id = i%GPU_NUM;
...
//	cudaSetDevice(GPU_id);
	...
	int id[PART_NUM/GPU_NUM];
	for(int j=0; j<PART_NUM/GPU_NUM; j++){
		id[j] = i+j*GPU_NUM;
