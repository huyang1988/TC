...
//		H_ERR(cudaMemcpy(src_degree, degree, vert_count*sizeof(index_t), cudaMemcpyHostToDevice) );
	...
	//
	vertex_t step_v = (vert_count+1)/GPU_NUM;
	vertex_t start_v = GPU_id * step_v;
	vertex_t end_v = (GPU_id + 1) * step_v;
	if(end_v>vert_count){end_v = vert_count+1;}

	index_t start_e = upperBegin[start_v];
	index_t end_e 	= upperBegin[end_v];
...
double time4 = wtime();
//	cout<<"total count = "<<count[GPU_id]<<endl;
	cout<<"GPU "<<GPU_id << " time = "<<time4-time2<<" seconds"<<endl;
