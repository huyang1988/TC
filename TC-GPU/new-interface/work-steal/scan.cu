...
cout<<"malloc adj for part "<<P<<endl;
...
void graph::DeviceCompute(int GPU_id, index_t Chunk_id){
	...
	index_t*	dev_begin	=gdata[GPU_id].begin;
	index_t*	dev_count	=gdata[GPU_id].count;
	Edge*		buffer		=gdata[GPU_id].EdgeBuffer[gdata[GPU_id].currentBuffer];
	gdata[GPU_id].currentBuffer	=(gdata[GPU_id].currentBuffer+1)%2;
	index_t currentBufferSize = BufferSize;
	if( Chunk_id==(upperEdgeCount-1)/BufferSize ){
...
cout<<"free beg"<<endl;
	...
	H_ERR(cudaFree(buffer0) );
	H_ERR(cudaFree(buffer1) );
