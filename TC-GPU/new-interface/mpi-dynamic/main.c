...
for(int n=0; n<r; n++){
	...
	gpuProc(graph_d,0,myrank,nprocs);
...
//cout<<"rank "<<myid<<" count "<<proc_count<<endl; 
	...
	double t2=wtime();
cout<<"rank "<<myrank<<" time "<<t2-t0<<endl; 
...
//cout<<"recv work from proc "<<status.MPI_SOURCE<<endl;
		MPI_Send(&work, 1, MPI_LONG_LONG, status.MPI_SOURCE, WORKTAG, MPI_COMM_WORLD);
...
//	double              result;
	...
	MPI_Status	status;
	for (;;) {
		MPI_Recv(&work, 1, MPI_LONG_LONG, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
