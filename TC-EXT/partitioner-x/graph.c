...
//			last_size[i] = partBegin[i][cc];
			last_size[i] = partBegin[i][cc] + 2*cc;
			rem += partEdgeCount[i] - partBegin[i][cc];
		}
...
//#pragma omp parallel for num_threads(56) schedule(dynamic,1024)
	...
	*/
	...
	FILE * m_File;
	m_File = fopen (meta_file, "wb");
	fwrite (metadata , sizeof(vertex_t), N+3 , m_File);
	fclose (m_File);
...
//#pragma omp parallel for num_threads(56) schedule(dynamic,1024)
	for(index_t i=0; i<upperEdgeCount; i++){
		...
		int j = 0;
		while(u>=metadata[3+j] && j<N-1){
			j++;
		}
		int k = 0;
		while(v>=metadata[3+k] && k<N-1){
			k++;
		}
//		int j = u/step;
//		int k = v/step;
