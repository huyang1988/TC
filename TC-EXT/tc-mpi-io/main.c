...
*/
	...
	double io_time = local->time_io;
	double cp_time = local->time_cp;
	double compute_time = local->time_compute;

	double global_io, global_cp, global_compute;
	...
	if(myrank==0){
		cout<<"total io time = "<<global_io<<endl;
		cout<<"total cp time = "<<global_cp<<endl;
		cout<<"total compute time = "<<global_compute<<endl;
	}
	else{
		cout<<"worker io time = "<<io_time<<endl;
		cout<<"worker cp time = "<<cp_time<<endl;
		cout<<"worker compute time = "<<compute_time<<endl;
	}
...
//cout<<"send work to proc "<<rank<<endl;	
	...
	while (rem_work>0){//( valid new work request ) {
		MPI_Recv(&result,	//        message buffer 
			1,              	// one data item 
			MPI_LONG_LONG,     	// of type double real 
...
//		cout<<"task "<<work<<" get count "<<result<<endl;
