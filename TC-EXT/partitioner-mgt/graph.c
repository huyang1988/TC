...
void graph::further_partition(){
	...
	_2DPART = (upperEdgeCount/PART_NUM + 2*vert_count - 1) / PartitionSize + 1;
//	_2DPART = (upperEdgeCount/PART_NUM - 1) / PartitionSize + 1;
	...
	for(int i=0; i<PART_NUM; i++){
		...
		for(int j=1; j<_2DPART; j++){
			int k = _2DPART * i + j;
			index_t part_total = partEdgeCount[i] + 2*vert_count; 
//			index_t part_total = partEdgeCount[i]; 
			index_t K = j*part_total/_2DPART;
			vertex_t index = BS(K, partBegin[i], 0, vert_count-1); // binary search
...
//			int t = N*i+j;
			...
			fwrite (p2Begin[i][j] , sizeof(index_t), temp_vert[t]+1, b_File);
			fclose (b_File);
