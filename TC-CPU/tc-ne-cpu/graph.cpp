...
#pragma omp parallel for num_threads(56) reduction(+:mycount) schedule (dynamic,512)
	for(index_t i=0; i<edge_count; i++){
		vertex_t A=head_list[i];
		vertex_t B=adj_list[i];
...
//			*/ 
	...
	cout<<"branch free bs version tc = "<<mycount/6<<endl;
}
...
#pragma omp parallel for num_threads(56) reduction(+:mycount) schedule (dynamic,512)
	...
	cout<<"bs version tc = "<<mycount/6<<endl;
}
//*/
/*
void graph::tc(){
...
//		if(m<32*n){
	...
	cout<<"hybrid version tc = "<<mycount/6<<endl;
}
*/
