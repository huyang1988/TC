...
void heap(vertex_t * src, vertex_t * dest, index_t left, index_t right) {
//      index_t i = left, j = right;
	index_t m = right - left;
	int power = 1;
	index_t temp = m;
	while(temp){
		temp = temp/2;
	...
	for(index_t i=0; i<m; i++){
		...
		index_t index = power * (2*(i-q+2)-1) /(2*q)-1;
		dest[left + i] = src[left + index];
	}
}
...
void graph::reheap(){
	...
	for(vertex_t i=0; i<vert_count; i++){
		index_t a=upperBegin[i];
		index_t b=upperBegin[i+1]-1;
		heap(upperAdj,heapAdj,a,b);
	}
}
...
void graph::rank_by_degree(){
	...
	cout<<"upper Edge Count= "<<upperEdgeCount<<"\n";
//printGraph(vert_count, upperHead, upperAdj, upperBegin);
}
